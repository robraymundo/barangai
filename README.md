...
* **The problem:** Local governments make hundreds of high-cost decisions each year — infrastructure, disaster preparedness, environmental planning, and budget allocation — largely from historical reports and intuition, with no way to preview outcomes or trade-offs before funds are committed. This slows progress on SDG 11 (Sustainable Cities and Communities), and related goals on infrastructure, climate action, and institutional resilience.
* **Our solution:** BarangAI builds a *digital twin* of a barangay from geospatial, demographic, environmental, and infrastructure data. Officials ask plain-language what-if questions ("What if we build an evacuation center here?" or "What if this road becomes one-way?") and get predicted outcomes — flood reduction, cooling, carbon absorption, cost, and beneficiaries — alongside vulnerability scoring, AI-ranked budget priorities, and a live community resilience score, all with a clear explanation of the reasoning.
...
* **Cloud Firestore (Firebase)** — Persistence for scenario history and the resilience-score timeline, with Firebase Authentication planned to support collaborative, multi-user access.
